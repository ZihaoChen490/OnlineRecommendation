...
def deal(BuyOrder,SellOrder,TransactionAuction,StockList,stk_code,Traders):
    ...
    if len(Trader)==0:
        ...
        Trader,StockList=ContinuousAuctionMorning(StockList,Traders)
        return Traders+Trader,StockList
    
def ContinuousAuctionMorning(StockList,Traders):
    print(Traders)
...
def send_Traders(Traders,fdmap,fd):
    if len(Traders)>0:
        index=0
...
def Order_data(data,StockList,data_list,fdmap,fd):
    Traders=[]
    if data=="!$$$":
        Traders,StockList=ContinuousAuctionMorning(StockList,Traders)
        if len(Traders)>0:
            send_Traders(Traders,fdmap,fd)
...
def hookDataCheck(data):
    print(data)
    send_pack=data.encode("utf-8")
    Done, Send_sp_hook = ConnectTrader(send_pack, Tradeip2, Tradeport2)
    return Done,Send_sp_hook

def ConnectTrader(send_pack,Tradeip2="10.216.68.192",Tradeport2=31622):
    global Trader_list
    host = Tradeip
